...
describe("exceptional-risk-only-v1 assessment", () => {
  ...
  test("states the complete low-friction decision principles", () => {
    ...
    for (const text of [
      ...
      "financial transactions such as purchases, trades, transfers, or billing or account changes",
      "Routine, bounded metered provider or API consumption incurred while carrying out an authorised task is not by itself an exceptional financial transaction.",
      "Require human review when concrete evidence makes unusually material or unbounded spend plausible",
      "including cumulative spend from repeated operations",
      "assess the overall operation rather than only one invocation",
      "keys, access grants or revocations, firewall or security disabling, or destructive root changes",
      "Do not classify code as untrusted_code_or_remote_payload merely because it crosses a process, SSH, container, localhost, or service-account boundary.",
      "Agent-generated code implementing trusted root-human intent",
      "controlled localhost target or controlled service account",
      "This profile prevents consequential accidents",
      "Make sensible contextual inferences from the authorised task",
      "Ordinary local PHP, Node.js, Python, and shell helpers carrying out the authorised task default to allow.",
      "An interpreter, a /tmp path, chmod 600, or missing source/provenance information does not by itself make a helper dangerous or untrusted.",
      "not a blanket allowance for /tmp or interpreters",
      "rm -rf ../ with cwd /tmp targets the root parent",
      "require human review for that broad valuable-data destruction",
