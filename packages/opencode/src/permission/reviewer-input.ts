...
function shell(input: unknown): PermissionReviewValue | undefined {
  ...
  for (const segment of segments) {
    ...
    let first = words.find((word) => !/^[a-z_][a-z0-9_]*=/i.test(word))
    if (!first) continue
    first =
      first
        .replace(/^['"]|['"]$/g, "")
    ...
    if (knownCommands.has(name)) names.add(name)
  ...
  if (/(^|\s)(eval|exec)(?=$|\s)|\b(sh|bash|python|perl)\s+-c\b/i.test(command)) traits.add("dynamic-execution")
  ...
  return {
    kind: "shell",
    size: length(command),
    commandCount: segments.length <= 10 ? segments.length : "many",
    commands: [...names].sort(),
    traits: [...traits].sort(),
